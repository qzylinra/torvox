...
fn mcp_schema_matches_code() {
    ...
    // implementation in mcp-server/src/lib.rs, and vice-versa.
    ...
    let lib_path = std::path::Path::new(WORKSPACE).join("mcp-server/src/server.rs");
    ...
    assert!(
        !code_tools.is_empty(),
        "no tools found in list_tools() in mcp-server/src/server.rs"
    );
    ...
    assert!(
        ...
        "tools in mcp-server/src/lib.rs but missing from docs/api/mcp-schema.json:\n  {}",
    ...
    assert!(
        ...
        "tools in docs/api/mcp-schema.json but missing from mcp-server/src/lib.rs:\n  {}",
