...
impl Selection {
    ...
    pub fn expand_word(
        mut self,
        cell_at: impl Fn(u32, u32) -> Option<char>,
        options: ExpansionOptions,
    ) -> Self {
        if !options.bridge_whitespace
            && classify_touch(cell_at(self.start.row, self.start.col)) != TouchClass::Text
        {
            return self;
        }
