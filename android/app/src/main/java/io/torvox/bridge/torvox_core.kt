...
public interface TorvoxBridgeInterface {
    fun `echoCells`(`cells`: List<BridgeCell>): List<BridgeCell>
...
open class TorvoxBridge :
    Disposable,
    AutoCloseable,
    TorvoxBridgeInterface {
    /**
     * @suppress
     */
    @Suppress("UNUSED_PARAMETER")
    ...
    override fun `echoCells`(`cells`: List<BridgeCell>): List<BridgeCell> =
    ...
    override fun `getConfig`(): TerminalConfig =
    ...
    override fun `ping`(): kotlin.String =
 ...
 * @suppress
 */
public object FfiConverterTypeTorvoxBridge : FfiConverter<TorvoxBridge, Long> {
    override fun lower(value: TorvoxBridge): Long = value.uniffiCloneHandle()

    override fun lift(value: Long): TorvoxBridge = TorvoxBridge(UniffiWithHandle, value)

    override fun read(buf: ByteBuffer): TorvoxBridge = lift(buf.getLong())

    override fun allocationSize(value: TorvoxBridge) = 8UL
 ...
 * @suppress
 */
public object FfiConverterTypeBridgeCell : FfiConverterRustBuffer<BridgeCell> {
    override fun read(buf: ByteBuffer): BridgeCell =
        BridgeCell(
...
sealed class TerminalException : kotlin.Exception() {
    class PtyException(
    ...
    companion object ErrorHandler : UniffiRustCallStatusErrorHandler<TerminalException> {
        override fun lift(error_buf: RustBuffer.ByValue): TerminalException = FfiConverterTypeTerminalError.lift(error_buf)
    }
}
 ...
 * @suppress
 */
public object FfiConverterTypeTerminalError : FfiConverterRustBuffer<TerminalException> {
    override fun read(buf: ByteBuffer): TerminalException =
        when (buf.getInt()) {
            1 -> {
                TerminalException.PtyException(
                    FfiConverterString.read(buf),
                )
            }

            2 -> {
                TerminalException.InvalidConfig(
                    FfiConverterString.read(buf),
                )
            }
    ...
    override fun allocationSize(value: TerminalException): ULong =
        when (value) {
            is TerminalException.PtyException -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL +
                    FfiConverterString.allocationSize(value.`detail`)
            )

            is TerminalException.InvalidConfig -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL +
                    FfiConverterString.allocationSize(value.`detail`)
            )
 ...
 * @suppress
 */
public object FfiConverterTypeTerminalEvent : FfiConverterRustBuffer<TerminalEvent> {
    override fun read(buf: ByteBuffer): TerminalEvent =
        when (buf.getInt()) {
            1 -> {
                TerminalEvent.Bell
            }
    ...
    override fun allocationSize(value: TerminalEvent) =
        when (value) {
            is TerminalEvent.Bell -> {
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                (
                    4UL
                )
 ...
 * @suppress
 */
public object FfiConverterSequenceTypeBridgeCell : FfiConverterRustBuffer<List<BridgeCell>> {
    override fun read(buf: ByteBuffer): List<BridgeCell> {
        val len = buf.getInt()
        return List<BridgeCell>(len) {
            FfiConverterTypeBridgeCell.read(buf)
        }
    }

    override fun allocationSize(value: List<BridgeCell>): ULong {
        val sizeForLength = 4UL
        val sizeForItems = value.map { FfiConverterTypeBridgeCell.allocationSize(it) }.sum()
        return sizeForLength + sizeForItems
    }

    override fun write(
        value: List<BridgeCell>,
        buf: ByteBuffer,
    ) {
        buf.putInt(value.size)
        value.iterator().forEach {
            FfiConverterTypeBridgeCell.write(it, buf)
        }
    }
}
