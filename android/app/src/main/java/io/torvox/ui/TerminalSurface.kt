...
class TerminalSurface
    ...
    constructor(
        ...
        fun renderSnapshotFromBuffer(
            ...
            val rows = bb.getInt(0)
            val cols = bb.getInt(4)
            val cursorRow = bb.getInt(8)
            val cursorCol = bb.getInt(12)
            val cursorVisible = buffer[16].toInt() != 0
            val n = count.coerceAtMost(rows * cols)
            val cells = kotlin.collections.ArrayList<CellSnapshot>(n)
            for (i in 0 until n) {
                val off = 20 + i * 12
                cells.add(CellSnapshot(bb.getInt(off), bb.getInt(off + 4), bb.getInt(off + 8)))
            }
            snapshotLock.lock()
            try {
                snapshotRows = rows
            ...
            // Queue UI-thread draw — uses lockCanvas on UI thread thread for safety
            mainHandler.post { doCanvasDraw() }
        }

        private fun doCanvasDraw() {
            ...
            try {
                ...
                val localRows: Int
                val localCols: Int
                val localCursorRow: Int
                val localCursorCol: Int
                val localCursorVisible: Boolean
                val localCells: kotlin.collections.ArrayList<CellSnapshot>?
                snapshotLock.lock()
                try {
                    localRows = snapshotRows
                    localCols = snapshotCols
                    localCursorRow = snapshotCursorRow
                    localCursorCol = snapshotCursorCol
                    localCursorVisible = snapshotCursorVisible
                    localCells = snapshotCells
                } finally {
                    snapshotLock.unlock()
                }
                if (localCells == null || localCells.isEmpty()) return
                val cellWidth = viewWidth / localCols
                val cellHeight = viewHeight / localRows
                val bgPaint =
                ...
                // Phase 1: Background — coalesce same-bg cells horizontally
                for (row in 0 until localRows) {
                    var col = 0
                    while (col < localCols) {
                        val idx = row * localCols + col
                        if (idx >= localCells.size) break
                        val bg = localCells[idx].bg
                        var runEnd = col + 1
                        while (runEnd < localCols) {
                            val nextIdx = row * localCols + runEnd
                            if (nextIdx >= localCells.size || localCells[nextIdx].bg != bg) break
                            runEnd++
                        }
                        bgPaint.color = bg
                        canvas.drawRect(col * cellWidth, row * cellHeight, runEnd * cellWidth, (row + 1) * cellHeight, bgPaint)
                        col = runEnd
                    }
                }
                // Phase 2: Text — group same-fg chars into String runs
                for (row in 0 until localRows) {
                    var col = 0
                    while (col < localCols) {
                        val idx = row * localCols + col
                        if (idx >= localCells.size) break
                        val cell = localCells[idx]
                        if (cell.cp == 32 || cell.cp == 0) {
                            col++
                            continue
                        }
                        val run = StringBuilder(localCols - col)
                        var runEnd = col
                        while (runEnd < localCols) {
                            val ri = row * localCols + runEnd
                            if (ri >= localCells.size) break
                            val rc = localCells[ri]
                            if (rc.cp == 32 || rc.cp == 0 || rc.fg != cell.fg) break
                            run.appendCodePoint(rc.cp)
                            runEnd++
                        }
                        if (run.isNotEmpty()) {
                            textPaint.color = cell.fg
                            canvas.drawText(run, 0, run.length, col * cellWidth, row * cellHeight + baseline, textPaint)
                        }
                        col = runEnd
                    }
                }
                ...
                if (localCursorVisible) {
                    ...
                    canvas.drawRect(
                        localCursorCol * cellWidth,
                        localCursorRow * cellHeight,
                        (localCursorCol + 1) * cellWidth,
                        (localCursorRow + 1) * cellHeight,
                        cursorPaint,
                    )
                }
            } finally {
        ...
        // Stored snapshot data for onDraw-based Canvas rendering (UI thread)
        private val snapshotLock =
            java.util.concurrent.locks
                .ReentrantLock()
        private var snapshotRows: Int = 0
        private var snapshotCols: Int = 0
        private var snapshotCursorRow: Int = 0
        private var snapshotCursorCol: Int = 0
        private var snapshotCursorVisible: Boolean = false
        private var snapshotCells: kotlin.collections.ArrayList<CellSnapshot>? = null
