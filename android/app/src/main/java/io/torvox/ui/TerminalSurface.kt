...
class TerminalSurface
    ...
    constructor(
        ...
        fun handleLongPress(
            ...
            val bridge = viewModel?.runtime?.bridge()
            val scrollbackLength = bridge?.scrollbackLength()?.toInt() ?: 0
            val lineRaw = bridge?.scrollbackLine((scrollbackLength - scrollOffset + row).toUInt())
            val lineTimedOut = lineRaw == null
            val line = lineRaw ?: ""
            val isOnEmptyArea =
                if (lineTimedOut) {
                    false
                } else {
                    col >= line.length || line.isEmpty()
                }
            val isOnWhitespace = !lineTimedOut && !isOnEmptyArea && col < line.length && line[col].isWhitespace()
