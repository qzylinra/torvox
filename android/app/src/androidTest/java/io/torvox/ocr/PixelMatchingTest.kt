...
import io.torvox.buildTemplate
import io.torvox.cellBottom
import io.torvox.cellRight
import io.torvox.cellX
import io.torvox.cellY
import io.torvox.decodeRgbaToPixels
import io.torvox.extractCell
import io.torvox.getBridge
import io.torvox.lastGridRowsDensity
import io.torvox.matchConfidence
import io.torvox.rowProfile
...
class PixelMatchingTest {
    ...
    fun pixel_match_after_echo_command() {
        ...
        bridge.writeToPty("echo 'HELLO_TORVOX_PIXEL'\n".toByteArray())
        Thread.sleep(3000)

        val dataText = bridge.getTerminalText()
        assertTrue(
            "TIER1 GATE: HELLO_TORVOX_PIXEL must appear in data text",
            dataText != null && dataText.contains("HELLO_TORVOX_PIXEL"),
        )
