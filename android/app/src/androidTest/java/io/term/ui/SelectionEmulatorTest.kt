...
class SelectionEmulatorTest {
    ...
    private fun captureFrame(): io.term.PixelFrame {
        val bridge = composeTestRule.getBridge() ?: throw AssertionError("Bridge is null")
        bridge.saveTestFrame(dataDir)
        return io.term.decodeRgbaToPixels(File(dataDir, "test_frame.rgba"))
    }
    ...
    private fun getSelectedText(): String? {
        ...
        composeTestRule.activityRule.scenario.onActivity { activity ->
            ...
            if (content != null) {
                val surface = io.term.findTerminalSurface(activity)
                if (surface is io.term.ui.TerminalSurface) {
    ...
    fun longPressOnWordCharacter_selectsWordWithInvertedColors() {
        ...
        for (c in col until col + 5) {
            val actual = io.term.extractCell(selFrame, c, line, gridCols, gridRows)
            val tmpl = io.term.extractCell(templateFrame, c, line, gridCols, gridRows)
            val conf = io.term.matchConfidence(actual, tmpl)
    ...
    fun selectionColors_matchTheme_notHardcoded() {
        ...
        val gridRows = composeTestRule.getBridge()!!.getGridRows()
        val gridCols = composeTestRule.getBridge()!!.getGridCols()
        val selCell = io.term.extractCell(selFrame, col, line, gridCols, gridRows)
        val tmplCell = io.term.extractCell(templateFrame, col, line, gridCols, gridRows)
        val conf = io.term.matchConfidence(selCell, tmplCell)
    ...
    fun selectionHighlight_visibleOnDarkTheme() {
        ...
        val ratio = io.term.analyzeNonBlackRatio(selFrame)
    ...
    fun selectionAcrossMultiline_highlightSpansMultipleRows() {
        ...
        Thread.sleep(500)
        bridge.saveTestFrame(dataDir)
        val selFrame = io.term.decodeRgbaToPixels(File(dataDir, "test_frame.rgba"))
        saveScreenshot("multiline-highlight")
        for (r in line1..min(line1 + 2, gridRows - 1)) {
            val selCell = io.term.extractCell(selFrame, 0, r, gridCols, gridRows)
            val tmplCell = io.term.extractCell(templateFrame, 0, r, gridCols, gridRows)
            val conf = io.term.matchConfidence(selCell, tmplCell)
    ...
    fun selectionHighlight_invertsForegroundBackground() {
        ...
        val gridRows = composeTestRule.getBridge()!!.getGridRows()
        val gridCols = composeTestRule.getBridge()!!.getGridCols()
        val selPixels = io.term.extractCell(selFrame, col, line, gridCols, gridRows)
        val tplPixels = io.term.extractCell(templateFrame, col, line, gridCols, gridRows)
        val conf = io.term.matchConfidence(selPixels, tplPixels)
    ...
    fun selectionOCR_verifiesContent() {
        ...
        val bitmap = io.term.decodeRgbaToBitmap(frameFile)
        try {
    ...
    fun selectionOnColoredText_preservesColorInversion() {
        ...
        val gridRows = composeTestRule.getBridge()!!.getGridRows()
        val gridCols = composeTestRule.getBridge()!!.getGridCols()
        val tmplPixels = io.term.extractCell(templateFrame, col, line, gridCols, gridRows)
        val selPixels = io.term.extractCell(selFrame, col, line, gridCols, gridRows)
        val conf = io.term.matchConfidence(selPixels, tmplPixels)
    ...
    fun selectionBoundaryExactMatch() {
        ...
        io.term.extractCell(selFrame, col, line, gridCols, gridRows)
        io.term.extractCell(selFrame, max(0, col - 1), line, gridCols, gridRows)
    ...
    fun selectionOnPartialLine_rightSideNotSelected() {
        ...
        val unselActual = io.term.extractCell(selFrame, unselCol, line, gridCols, gridRows)
        val unselTmpl = io.term.extractCell(templateFrame, unselCol, line, gridCols, gridRows)
        val unselConf = io.term.matchConfidence(unselActual, unselTmpl)
    ...
    fun selectionHighlight_notShownWhenActiveFalse() {
        ...
        val gridCols = composeTestRule.getBridge()!!.getGridCols()
        for (c in col..col + 6) {
            val actual = io.term.extractCell(inactiveFrame, c, line, gridCols, gridRows)
            val tmpl = io.term.extractCell(templateFrame, c, line, gridCols, gridRows)
            val conf = io.term.matchConfidence(actual, tmpl)
    ...
    fun selectionLineExact_boundaryOnLeftEdge() {
        ...
        val gridRows = composeTestRule.getBridge()!!.getGridRows()
        val gridCols = composeTestRule.getBridge()!!.getGridCols()
        val leftCell = io.term.extractCell(selFrame, 0, line, gridCols, gridRows)
        val leftTmpl = io.term.extractCell(templateFrame, 0, line, gridCols, gridRows)
        val conf = io.term.matchConfidence(leftCell, leftTmpl)
    ...
    fun selectionColorDoesNotBleed_toAdjacentCell() {
        ...
        if (bleedCol < gridCols) {
            val adjActual = io.term.extractCell(selFrame, bleedCol, line, gridCols, gridRows)
            val adjTmpl = io.term.extractCell(templateFrame, bleedCol, line, gridCols, gridRows)
            val adjConf = io.term.matchConfidence(adjActual, adjTmpl)
    ...
    fun fastSelectDeselect_noGpuArtifacts() {
        ...
        val ratio = io.term.analyzeNonBlackRatio(frame)
    ...
    fun selectionOnAlternateBg_inversionLooksCorrect() {
        ...
        val gridCols = composeTestRule.getBridge()!!.getGridCols()
        val selPixels = io.term.extractCell(selFrame, col, line, gridCols, gridRows)
        val tplPixels = io.term.extractCell(templateUrlFrame, col, line, gridCols, gridRows)
        val conf = io.term.matchConfidence(selPixels, tplPixels)
