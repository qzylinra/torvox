...
class SelectionEmulatorTest {
    ...
    fun selection_ocr_afterEchoCommand_detectsText() {
        composeTestRule.waitForSession()
        val bridge = composeTestRule.getBridge() ?: throw AssertionError("Bridge is null")
        bridge.writeToPty("echo 'SEL_OCR_456'\n".toByteArray())
        Thread.sleep(3000)

        val dataText = bridge.getTerminalText()
        assertTrue(
            "SEL_OCR_456 must appear",
            dataText != null && dataText.contains("SEL_OCR_456"),
        )
        ...
        assertTrue(
            "OCR must detect SEL_OCR_456: $ocrText",
            ocrText.contains("SEL_OCR_456"),
        )
    }
