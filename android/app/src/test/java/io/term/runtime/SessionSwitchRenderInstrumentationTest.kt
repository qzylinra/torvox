 ...
 *
 * Fix under test: `TerminalRuntime.switchSessionInternal` performs a SYNCHRONOUS
 * first `render()` of the newly-active session before the event-driven render
 * thread takes over (TerminalRuntime.kt:721-751). This guarantees the
 ...
 * None of that is available under Robolectric/JVM, and the native `NativeBridge`
...
class SessionSwitchRenderInstrumentationTest {
    ...
    fun switch_session_internal_render_seam_exists() {
        ...
        val hasSwitchSeam =
            TerminalRuntime::class.java.declaredMethods.any { name ->
    ...
    @Ignore(
        "Requires a live Vulkan GPU surface + native NativeBridge; verified by code " +
            "inspection at TerminalRuntime.kt:721-751 (synchronous bridge.render() before the " +
    ...
    fun switch_session_renders_synchronously_before_event_loop() {
        ...
        // ANativeWindow pointer and NativeBridge is a native JNA binding.
