...
class FfiContractTest {
    ...
    fun shell_custom_longPath() {
        val path = "/data/data/io.term/files/bootstrap/usr/bin/bash"
        val w = WireWriter()
    ...
    fun terminalConfig_fieldOrderMatchesRust() {
        ...
        val config =
            TerminalConfig(
                ...
                home = "/data/data/io.term",
                user = "u0_a123",
                path = "/system/bin",
                workingDirectory = "/data/data/io.term/files",
                prefix = "/data/data/io.term/files",
            )
        ...
        // strings
        assertEquals("/data/data/io.term", r.readString())
        assertEquals("u0_a123", r.readString())
        assertEquals("/system/bin", r.readString())
        assertEquals("/data/data/io.term/files", r.readString())
        assertEquals("/data/data/io.term/files", r.readString())
    }
    ...
    fun terminalConfig_fullRoundTrip_preservesAllFields() {
        val original =
            TerminalConfig(
                ...
                home = "/data/data/io.term/files",
                user = "u0_a123",
                path = "/system/bin:/data/data/io.term/files/bootstrap/usr/bin",
                workingDirectory = "/data/data/io.term/files",
                prefix = "/data/data/io.term/files",
            )
