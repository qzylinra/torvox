...
class FontFileInstallTest {
    ...
    fun fontFamilyIsSavedAfterInstall() = runTest {
        val settingsProvider = io.torvox.settings.SettingsDataStoreProvider(context)
        val repository = io.torvox.settings.SettingsRepository(settingsProvider)
    ...
    private fun createViewModel(): io.torvox.TerminalViewModel {
        val settingsProvider = io.torvox.settings.SettingsDataStoreProvider(context)
        val repository = io.torvox.settings.SettingsRepository(settingsProvider)
        val runtime = io.torvox.runtime.TorvoxRuntime(context, repository)
        return io.torvox.TerminalViewModel(context, repository, runtime)
    }
}
