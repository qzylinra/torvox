...
│   │   ├── ghostty_terminal.rs # GhosttyTerminal (libghostty-vt, channel-based)
...
│   └── Cargo.toml
│
├── torvox-fuzz/ # 模糊测试 (4 targets: vt_parser, osc_parse, grid_resize, keyboard_input)
│   └── fuzz/
│       ├── fuzz_targets/
│       │   ├── fuzz_vt_parser.rs
│       │   ├── fuzz_osc_parse.rs
...
│ │ │ └── bridge/
│ │ │     └── TorvoxBridge.kt # JNA 桥接 (WireReader 协议)
│   │   └── build.gradle.kts
│   ├── gradle/
│   ├── settings.gradle.kts
│   └── build.gradle.kts
...
| libghostty-vt | 0.1.1 (patch) | VT 解析器 (GhosttyTerminal, channel-based) |
...
│ 会话 1                                                           │
│ ┌──────────────┐  ┌──────────────┐  ┌────────────────────┐      │
│ │ PTY Reader │─►│ Ghostty- │─►│ Grid │ │
│ │ (block read) │ │ Terminal │ │ (Arc<Mutex>) │ │
│ └──────────────┘  └──────────────┘  └────────┬───────────┘      │
│                                               │                  │
│ ┌──────────────┐  ┌──────────────┐           │                  │
...
| **VT 解析器** | 手写 Java FSM (2617行) | libvterm (C/JNI) | libghostty-vt (GhosttyTerminal, Ghostty 使用) |
...
| **序列化** | Java Serializable | C struct | serde derive (生产) + postcard 1.1 (测试回放) |
