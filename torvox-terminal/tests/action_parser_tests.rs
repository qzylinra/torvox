...
use torvox_terminal::action_parser::CsiSeq;
...
fn sized(r: u32, c: u32) -> torvox_terminal::ghostty_terminal::GhosttyTerminal {
    torvox_terminal::ghostty_terminal::GhosttyTerminal::new(r, c, 1000).expect("terminal")
}
...
fn l2_csi_sgr_multi_params() {
    ...
    assert!(snap.cells[0].fg[0] > 0.0, "red fg");
    assert!(snap.cells[0].bg[1] > 0.0, "green bg");
}
