...
//! `torvox-terminal` (feature `live`), which is what powers end-to-end testing
...
use torvox_mcp::{SessionStore, serve_tcp, serve_unix};

#[derive(Parser)]
#[command(
    name = "torvox-mcp",
    about = "MCP server exposing Torvox terminal sessions to AI agents"
)]
struct Cli {
    ...
    /// Spawn a real shell session via torvox-terminal and serve it (feature `live`).
    #[cfg(feature = "live")]
    #[arg(long)]
    live: bool,
...
impl SessionStore for NoOpStore {
    fn read(&self, req: torvox_mcp::ReadRequest) -> Result<torvox_mcp::ReadResponse, String> {
        match req {
            torvox_mcp::ReadRequest::Sessions => {
...
use torvox_mcp::SessionInfo;
#[cfg(feature = "live")]
use torvox_mcp::live::LiveShellStore;
#[cfg(feature = "mock")]
use torvox_mcp::mock::MockStore;

fn main() {
    let cli = Cli::parse();

    let filter = format!(
        "torvox_mcp={},torvox_terminal={},warn",
        cli.log.as_str(),
        cli.log.as_str()
    );
    ...
    let store: Arc<dyn SessionStore> = {
        #[cfg(feature = "live")]
        {
            if cli.live {
                ...
                {
                    if cli.mock {
                        log::info!("torvox-mcp: serving mock store");
    ...
    if let Some(addr) = &cli.tcp {
        if let Err(e) = serve_tcp(addr, store, write_consent) {
            eprintln!("torvox-mcp: server error: {e}");
            std::process::exit(1);
        }
        ...
        eprintln!("torvox-mcp: either --socket PATH or --tcp ADDR is required");
        std::process::exit(1);
    }
}
