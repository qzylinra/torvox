...
//!
//! This drives the actual `mcp-server` Android binary (built with `--features
...
//! Run with:
//! ```text
//! TORVOX_EMULATOR_TEST=1 cargo test --package mcp-server --features emulator \
//!     --test emulator_e2e
//! ```
...
fn android_bin() -> String {
    ...
    workspace
        .join("target/x86_64-linux-android/debug/mcp-server")
        .to_string_lossy()
        .into_owned()
}
...
fn emulator_full_protocol_round_trip() {
    ...
    assert!(
        ...
        "android binary not found at {bin}; build with: cargo ndk --target x86_64 --platform 21 build --package mcp-server --features mock"
    );
    ...
    adb_run(&["push", &bin, "/data/local/tmp/mcp-server"]);
    adb_run(&["shell", "chmod", "755", "/data/local/tmp/mcp-server"]);
    adb_run(&["shell", "pkill", "-f", "mcp-server"]);
    // Wait until any previous instance has actually exited.
    std::thread::sleep(Duration::from_millis(800));
    let port = 8700 + (std::process::id() % 300) as u16;
    let launch = format!(
        "nohup setsid /data/local/tmp/mcp-server --tcp 127.0.0.1:{port} --mock --write-consent > /data/local/tmp/mcp_run.log 2>&1 < /dev/null &"
    );
...
impl Drop for DeviceCleanup {
    fn drop(&mut self) {
        adb_run(&["shell", "pkill", "-f", "mcp-server"]);
        // removing all forwards is safest; tests use a small port range.
