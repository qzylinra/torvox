//! Integration tests for `mcp-server` exercising the full public surface
...
fn spawn_server(
    ...
    let socket = dir.join(format!("mcp-server-test-{}-{}.sock", std::process::id(), n));
...
fn concurrent_tcp_requests() {
    ...
    let socket_path = {
        ...
        dir.join(format!("mcp-server-concurrent-{n}.sock"))
    };
...
fn large_payload_through_tcp() {
    ...
    let socket_path = {
        let dir = std::env::temp_dir();
        let n = std::process::id();
        dir.join(format!("mcp-server-large-{n}.sock"))
    };
    let _ = std::fs::remove_file(&socket_path);
    let serve_socket = socket_path.clone();
    let handle = std::thread::spawn(move || {
...
fn client_disconnect_no_panic() {
    ...
    let socket_path = {
        let dir = std::env::temp_dir();
        let n = std::process::id();
        dir.join(format!("mcp-server-disconnect-{n}.sock"))
    };
    let _ = std::fs::remove_file(&socket_path);
    let serve_socket = socket_path.clone();
    let handle = std::thread::spawn(move || {
...
fn write_round_trip_through_tcp() {
    ...
    let socket_path = {
        let dir = std::env::temp_dir();
        let n = std::process::id();
        dir.join(format!("mcp-server-writer-{n}.sock"))
    };
    let _ = std::fs::remove_file(&socket_path);
    let serve_socket = socket_path.clone();
    let handle = std::thread::spawn(move || {
