//! Real `SessionStore` backend built on `terminal-engine`.
//!
...
//! cargo feature so the default build of `mcp-server` stays dependency-light
//! (no `terminal-engine` / ghostty) and CI stays fast.
//!
//! # Requirements
...
/// A live session store backed by real `terminal-engine::Session` instances.
