...
unsafe extern "C" {
    fn bridge_poll_all(handle: i64) -> i64;
}
...
pub struct NativeBridge {
    ...
    pub(crate) session: std::sync::Mutex<
        Option<std::sync::Arc<std::sync::Mutex<terminal_engine::session::Session>>>,
    >,
    ...
    pub(crate) key_cmd_tx:
        std::sync::Mutex<Option<flume::Sender<terminal_engine::ghostty_terminal::Command>>>,
}

impl NativeBridge {
    pub(crate) fn active_session(
        &self,
    ) -> Result<std::sync::Arc<std::sync::Mutex<terminal_engine::session::Session>>, BridgeError>
    {
    ...
    fn shell_env(&self) -> terminal_engine::shell_env::ShellEnv {
        if self.config.home.is_empty() {
            terminal_engine::shell_env::ShellEnv::default()
        } else {
            ...
            terminal_engine::shell_env::ShellEnv {
    ...
    fn process_session_for_render(
        &self,
    ) -> Result<(bool, terminal_engine::ghostty_terminal::GridSnapshot), BridgeError> {
...
where
    F: FnOnce(&NativeBridge) -> Result<T, BridgeError>,
{
    let ptr = handle as *const NativeBridge;
    if ptr.is_null() {
        return Err(TerminalError::InvalidConfig {
            detail: "null bridge handle".to_string(),
        });
    }
    // SAFETY: ptr is non-null (checked above) and was created from a Box<NativeBridge>
...
impl NativeBridge {
    ...
    pub fn ping(&self) -> Result<String, BridgeError> {
        let ptr = self as *const NativeBridge;
        log::info!(
            "ping: self={:p}, aligned={}",
            ptr,
    ...
    pub fn spawn_terminal(&self, _rows: u32, _cols: u32) -> Result<i32, BridgeError> {
        let shell: terminal_core::config::Shell = self.config.shell.clone().into();
        let shell_path = match &shell {
            terminal_core::config::Shell::SystemDefault => "/system/bin/sh",
            terminal_core::config::Shell::Custom(path) => path.as_str(),
        };
    ...
    pub fn save_test_frame_with_selection(
        ...
        if let Some(surface) = surface_guard.as_mut() {
            if active && start_row >= 0 && end_row >= 0 {
                surface.set_selection(Some(gpu_renderer::gpu::SelectionRange {
                    start_row,
                    start_col,
                    end_row,
                    end_col,
                    active: true,
                    mode: terminal_core::selection::SelectionMode::from_u8(mode),
                    origin: None,
                    is_empty: false,
                }));
            } else {
    ...
    pub fn get_theme_names(&self) -> String {
        terminal_core::config::Theme::all_built_in()
            .into_iter()
            .map(|t| t.name)
            .collect::<Vec<_>>()
            .join(LIST_SEPARATOR)
    ...
    pub fn set_extra_font_paths(&self, _paths: Vec<String>) {
        #[cfg(target_os = "android")]
        {
            ...
            gpu_renderer::font::set_extra_font_paths(path_bufs);
        }
    }
        ...
        if let Some(surface) = surface_guard.as_mut() {
            if active && start_row >= 0 && end_row >= 0 {
                let range = crate::bridge::selection::range_from(
                    terminal_core::selection::SelectionAnchor {
                    ...
                    terminal_core::selection::SelectionMode::from_u8(mode),
                    None,
                    false,
                );
    ...
    pub fn is_cell_empty(&self, row: u32, col: u32) -> bool {
        if let Ok(mut surface_guard) = self.surface.lock()
            && let Some(_surface) = surface_guard.as_mut()
            && let Ok(guard) = self.session.lock()
            && let Some(session_arc) = guard.as_ref()
            && let Ok(session) = session_arc.lock()
        {
            ...
            for r in start_row..=end_row {
                ...
                for c in cstart..=cend {
                    ...
                    if let Some(cell) = snapshot.cells.get(idx)
                        && let Some(ch) = char::from_u32(cell.codepoint)
                        && ch != '\0'
                        && !ch.is_whitespace()
                    {
                        return true;
    ...
    pub fn expand_and_set_selection(
        ...
        let mode_enum = terminal_core::selection::SelectionMode::from_u8(mode);
        let mut surface_guard = lock_surface!(self);
        if let Some(surface) = surface_guard.as_mut() {
            if let Ok(guard) = self.session.lock()
                && let Some(session_arc) = guard.as_ref()
            ...
            {
                let snapshot = session.terminal().take_snapshot();
                let anchor = terminal_core::selection::SelectionAnchor { row, col };
                let (start, end) = crate::bridge::selection::expand_at(
                    &snapshot.cells,
                    snapshot.cols,
                    anchor,
                    mode_enum,
                    terminal_core::selection::ExpansionOptions::default(),
                );
    ...
    pub fn set_selection_endpoint(
        ...
        let mode_enum = terminal_core::selection::SelectionMode::from_u8(params.mode);
        let mut surface_guard = lock_surface!(self);
        if let Some(surface) = surface_guard.as_mut() {
            if let Ok(guard) = self.session.lock()
                && let Some(session_arc) = guard.as_ref()
                && let Ok(session) = session_arc.lock()
            {
                ...
                let fixed = terminal_core::selection::SelectionAnchor {
                    row: params.other_row.max(0) as u32,
                    col: params.other_col.max(0) as u32,
                };
                let moved = terminal_core::selection::SelectionAnchor {
                    row: params.anchor_row.max(0) as u32,
                    col: params.anchor_col.max(0) as u32,
                };

                let (new_start, new_end) =
                    if mode_enum == terminal_core::selection::SelectionMode::Word {
    ...
    pub fn set_search_highlights(&self, serialized: Vec<u8>) -> Result<(), BridgeError> {
        ...
        for _ in 0..count.max(0) {
            ...
            highlights.push(gpu_renderer::gpu::SearchHighlight {
                row,
                start_col,
                end_col_exclusive,
    ...
    pub fn process_key_event(
        ...
        let rx = {
            ...
            match guard.as_ref() {
                Some(cmd_tx) => {
                    terminal_engine::ghostty_terminal::GhosttyTerminal::key_encode_submit_via(
                        cmd_tx,
                        key_code,
                        modifiers as u16,
                        action,
    ...
    pub fn set_cursor_style(&self, style: String) -> Result<(), BridgeError> {
        let cursor_style = match style.as_str() {
            "bar" => terminal_core::cursor::CursorStyle::Bar,
            "underline" => terminal_core::cursor::CursorStyle::Underline,
            _ => terminal_core::cursor::CursorStyle::Block,
        };
        let mut surface_guard = lock_surface!(self);
        if let Some(surface) = surface_guard.as_mut() {
