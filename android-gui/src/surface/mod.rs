...
use gpu_renderer::gpu::GpuContext;
use gpu_renderer::gpu::SearchHighlight;
use gpu_renderer::gpu::SelectionRange;
use thiserror::Error;
...
impl From<gpu_renderer::gpu::GpuError> for SurfaceError {
    fn from(e: gpu_renderer::gpu::GpuError) -> Self {
...
pub struct AndroidSurface {
    ...
    atlas_width: u32,
    atlas_height: u32,
    theme: terminal_core::config::Theme,
    rows: u32,
    cols: u32,
    ...
    last_cursor_row: u32,
    last_cursor_col: u32,
    cursor_style: terminal_core::cursor::CursorStyle,
    blink_phase: bool,
    blink_enabled: bool,
    blink_speed_ms: u32,
    ...
    prev_cells: Vec<terminal_engine::ghostty_terminal::CellSnapshot>,
...
struct FrameBuffers {
    ...
    instance_buffer: Vec<gpu_renderer::gpu::CellInstance>,
    ...
    cached_instances: Vec<gpu_renderer::gpu::CellInstance>,
...
impl AndroidSurface {
    pub fn new(rows: u32, cols: u32, scrollback_lines: u32, font_size: f32) -> Self {
        ...
        Self {
            ...
            atlas_width,
            atlas_height,
            theme: terminal_core::config::Theme::catppuccin_mocha(),
            rows,
            cols,
    ...
    pub fn set_theme(&mut self, theme: terminal_core::config::Theme) {
    ...
    pub fn theme(&self) -> &terminal_core::config::Theme {
        &self.theme
    }
}
...
mod tests {
    ...
    fn cell_to_line_populated() {
        let cells = vec![
            terminal_engine::ghostty_terminal::CellSnapshot {
                codepoint: 0x41, // 'A'
                graphemes: Vec::new(),
    ...
    fn cell_to_line_pads_short_input() {
        let cells = [terminal_engine::ghostty_terminal::CellSnapshot {
            codepoint: 0x41,
            ..Default::default()
        }];
        let line = cell_to_line(&cells, 5);
    ...
    fn line_to_text_preserves_colors() {
        let cell = terminal_engine::ghostty_terminal::CellSnapshot {
            codepoint: 0x41,
    ...
    fn set_theme_sets_render_requested() {
        ...
        let theme = terminal_core::config::Theme::catppuccin_mocha();
    ...
    fn restore_session_nul_becomes_space_and_trims_trailing_blanks() {
        use terminal_engine::ghostty_terminal::CellSnapshot;
        ...
        let snapshot = terminal_core::snapshot::SessionSnapshot {
    ...
    #[test]
    fn atlas_size_is_2048_square() {
        assert_eq!(ATLAS_WIDTH, 2048, "ATLAS_WIDTH must be 2048");
        assert_eq!(ATLAS_HEIGHT, 2048, "ATLAS_HEIGHT must be 2048");
    }

    #[test]
    fn kgp_atlas_width_matches_atlas() {
        assert_eq!(
            KGP_ATLAS_WIDTH, 2048,
