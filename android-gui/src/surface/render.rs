...
impl AndroidSurface {
    ...
    pub fn render(&mut self, scroll_offset: u32) -> Result<bool, SurfaceError> {
        ...
        let snapshot;
        {
            let session = self.session.as_ref().ok_or(SurfaceError::NoSession)?;
            let mut guard = session.lock().map_err(|_| SurfaceError::NoSession)?;
    ...
    fn render_inner(
        ...
        scroll_offset: u32,
        had_output: bool,
        mut snapshot: terminal_engine::ghostty_terminal::GridSnapshot,
    ) -> Result<bool, SurfaceError> {
        ...
        gpu_renderer::gpu::build_cell_instances_into(
            &snapshot,
            ...
            gpu_renderer::gpu::CellInstanceConfig {
                ...
                render_scale: gpu_renderer::gpu::RENDER_SCALE,
        ...
        if !snapshot.kgp_placements.is_empty() {
            ...
            let kgp_data: Vec<KgpImageData> = {
                let session_ref = self.session.as_ref().ok_or(SurfaceError::NoSession)?;
                let guard = session_ref.lock().map_err(|_| SurfaceError::NoSession)?;
                let session = &*guard;
            ...
            for img in &kgp_data {
                ...
                for p in &snapshot.kgp_placements {
                    if p.image_id == img.id {
                        let inst = gpu_renderer::gpu::KgpInstance::new(
...
mod tests {
    use super::*;
    use terminal_engine::ghostty_terminal::GridSnapshot;
