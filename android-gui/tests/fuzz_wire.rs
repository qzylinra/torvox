...
use android_gui_lib::bridge::{BridgeTheme, Shell, TerminalConfig};
use terminal_core::config::TerminalConfig as CoreConfig;

/// Core → Bridge conversion preserves all TerminalConfig fields
#[test]
fn core_to_bridge_rows_and_cols() {
    let core = CoreConfig {
        shell: terminal_core::config::Shell::SystemDefault,
        rows: 36,
        cols: 120,
...
fn core_to_bridge_custom_shell() {
    let core = CoreConfig {
        shell: terminal_core::config::Shell::Custom("/bin/zsh".into()),
        rows: 24,
        cols: 80,
    ...
    assert_eq!(
        back.shell,
        terminal_core::config::Shell::Custom("/bin/zsh".into())
    );
}
...
fn bridge_only_fields_lost_on_core_conversion() {
    let bridge = TerminalConfig {
        ...
        home: "/data/data/com.example/files".into(),
        user: "termuser".into(),
        path: "/system/bin:/system/xbin".into(),
        working_directory: "/data/data/com.example/files".into(),
        prefix: String::new(),
    };
...
fn bridge_theme_all_ansi_colors_round_trip() {
    let orig = terminal_core::config::Theme::dracula_plus();
    let bridge = BridgeTheme::from(orig.clone());
    let back: terminal_core::config::Theme = bridge.into();
