...
//! `nix develop --command cargo test -p android-gui --test gpu_vulkan_tests`
...
/// The  real `GpuContext` must initialize against Lavapipe Vulkan (the same
/// wgpu/Vulkan path used on a real Android GPU).
#[test]
fn gpu_context_inits_on_vulkan() {
    let _gpu = gpu_renderer::gpu::GpuContext::new_with_no_surface();
}
