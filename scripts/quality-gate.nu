...
print "[2/13] no_std build (torvox-core alloc only)"
...
print "[3/13] cargo clippy -- -D warnings"
...
print "[4/13] cargo test --workspace"
...
print "[5/13] cargo mutants (mutation testing)"
if (which cargo-mutants | length) > 0 {
    let mutants_result = (^cargo mutants --timeout 120 ignore | complete)
    if $mutants_result.exit_code != 0 {
        $failed = ($failed | append "cargo mutants")
        print "FAIL: cargo mutants below threshold"
    } else {
        print "OK: mutation score meets threshold"
    }
} else {
    print "SKIP: cargo-mutants not installed"
}

print "[6/13] cargo kani (formal verification)"
if (which cargo-kani | length) > 0 {
    let kani_result = (^cargo kani --manifest-path torvox-core/kani/Cargo.toml --enable-unstable | complete)
    if $kani_result.exit_code != 0 {
        $failed = ($failed | append "cargo kani")
        print "FAIL: cargo kani verification failed"
    } else {
        print "OK: Kani proofs verified"
    }
} else {
    print "SKIP: cargo-kani not installed"
}

print "[7/13] cargo audit"
...
print "[10/13] markdownlint-cli2"
if (which markdownlint-cli2 | length) > 0 {
    let result = (^markdownlint-cli2 '**/*.md' --ignore 'node_modules' --ignore 'target/**' --ignore 'libghostty-rs/**' --ignore '.github/**' --ignore 'docs/ADR/' | complete)
...
print "[12/13] Roborazzi visual regression tests"
if ((("android" | path exists) and (($env.JAVA_HOME? | default "") != "")) and (which gradle | length) > 0) {
    cd android
    let roborazzi_result = (^./gradlew roborazziDebug --quiet | complete)
    if $roborazzi_result.exit_code != 0 {
        $failed = ($failed | append "Roborazzi visual regression")
        print "FAIL: Roborazzi visual regression tests"
    } else {
        print "OK: Roborazzi tests passed"
    }
    cd ..
} else {
    print "SKIP: Roborazzi tests (no JAVA_HOME/android dir/gradle)"
}

print "[13/13] APK build (assembleDebug)"
if $has_android {
    cd android
