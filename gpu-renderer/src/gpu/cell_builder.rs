...
use terminal_core::selection::SelectionMode;
        ...
        for col in 0..cols {
            ...
            if cell.codepoint == 0 || cell.codepoint == 0x20 {
                ...
                let (quad_size, quad_origin) = if is_cursor {
                    ...
                    let cursor_alpha = match cursor_style {
                        terminal_core::cursor::CursorStyle::Block => CURSOR_BLOCK_ALPHA,
                        _ => CURSOR_LINE_ALPHA,
                    };
                    let cursor_bg = [
                        raw_cursor_bg[0],
                    ...
                    bg = cursor_bg;
                    match cursor_style {
                        terminal_core::cursor::CursorStyle::Block => {
                            ([cell_w, cell_h], [base_x, base_y])
                        }
                        terminal_core::cursor::CursorStyle::Bar => {
                            ([cell_w * CURSOR_BAR_WIDTH_RATIO, cell_h], [base_x, base_y])
                        }
                        terminal_core::cursor::CursorStyle::Underline => (
                ...
                for sg in &shaped {
                    ...
                    let (gfg_scoped, gbg_scoped) = if g_cursor {
                        ...
                        let (cursor_alpha, gfg_override) = match cursor_style {
                            terminal_core::cursor::CursorStyle::Block => (CURSOR_BLOCK_ALPHA, gbg),
            ...
            let (fg, bg) = if is_cursor {
                ...
                let (cursor_alpha, fg_override) = match cursor_style {
                    terminal_core::cursor::CursorStyle::Block => (CURSOR_BLOCK_ALPHA, bg),
