...
fn text_render_ocr_verify() {
    ...
    let png_path = std::env::temp_dir().join("text_ocr_test.png");
    ...
    assert!(
        cleaned.contains("Hello") || cleaned.contains("Renderer"),
        "Expected 'Hello Renderer' in OCR output, got: {cleaned:?}"
    );
